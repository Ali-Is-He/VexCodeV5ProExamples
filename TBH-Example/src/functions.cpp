...
float gain = 0.00025;      // VAIRABLE YOU WILL BE TUNING, This controls how fast/agressive the TBH algorithm and values that go to your flywheel are
...
const int GEARRATIO=5;    // Gear Ratio you are using on your flywheel. If you are using a rotation sensor on your flywheel axle keep this at 1, if you are using the built in motor encoders then set this to the gear ratio you use
...
int flywheelTBHManager()
{
  ...
  */
  while(true)
  {
    // currentVelocity=FlywheelMotors.velocity(velocityUnits::rpm)*GEARRATIO;  // Get the speed of the flywheel with the built in motor encoder, UNCOMMENT THIS LINE IF YOU ARE NOT USING A ROTATION SENSOR
    currentVelocity=flywheelRotationSensor.velocity(rpm);   // Get the speed of the flywheel with the rotation sensor on the flywheel axle
